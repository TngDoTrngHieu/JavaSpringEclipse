...
public class UserVipService {
	...
	public List<UserVips> findAll() {
		return repository.findAll();
	}

	public UserVips findById(Long id) {
		return repository.findById(id).orElseThrow(() -> new RuntimeException("User vip not found"));
	}

	public UserVips create(UserVips item) {
	...
	public UserVips update(Long id, UserVips payload) {
		...
		c.setExpireAt(payload.getExpireAt());
		c.setPayment(payload.getPayment());
		c.setUser(payload.getUser());
		c.setVipPackage(payload.getVipPackage());
