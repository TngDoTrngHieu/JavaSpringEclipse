	...
	public ResponseEntity<?> generateQuiz(@RequestBody Map<String, Object> body, Principal principal) {
		...
		if (!userService.isUserVip(principal.getName())) {
			return ResponseEntity.status(403)
					.body(Map.of("error", "Tính năng tạo câu hỏi AI hiện chỉ dành cho thành viên VIP."));
		}

		Object passageObj = body.get("passage");
		if (passageObj == null) {
			return ResponseEntity.badRequest()
					.body(Map.of("error", "Thiếu nội dung đầu vào: vui lòng gửi trường 'passage'."));
		}
		...
		try {
			...
			return ResponseEntity.status(500)
					.body(Map.of("error", "Hệ thống AI đang bận. Vui lòng thử lại sau ít phút."));
		}
	}
}
